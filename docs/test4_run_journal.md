...
| E2 | pending | e2effcf | fixes: finereg translations PROJECTED onto matched-normal span (null space zeroed; on a linear walk = along-walk axis), keep only on ≥3% worst-separation gain else FULL ROLLBACK; hybrid DA3 extraction for ALL 210 kf on the re-run path | | questions: serpenteado (hybrid now full-coverage)? finereg rolls back or pays? duplicates/holes? |
