...
class DA3_Streaming:
    def __init__(self, image_dir, save_dir, config):
        ...
        self.seed = 42
        # Device: read from config, fallback to auto-detect
        self.device = self.config.get("Model", {}).get("device", None)
    ...
    def process_single_chunk(self, range_1, chunk_idx=None, range_2=None, is_loop=False):
        ...
        with torch.no_grad():
            with torch.amp.autocast(device_type=self.device, dtype=self.dtype):
                ...
                # Debug: print conf before adjustment
                print(f"[DEBUG] Conf before -1.0: min={np.min(predictions.conf):.4f}, max={np.max(predictions.conf):.4f}, mean={np.mean(predictions.conf):.4f}")
                
                # Check if confidence is uniform (model didn't output meaningful confidence)
                if np.min(predictions.conf) == np.max(predictions.conf):
                    print("[INFO] Uniform confidence detected - model did not output meaningful confidence values")
                    print("[INFO] Using depth-based point cloud without confidence filtering")
                    # Set conf to a high value so all points pass the filter
                    predictions.conf = np.ones_like(predictions.conf)
                else:
                    predictions.conf -= 1.0
                
                # Debug: print conf after adjustment
                print(f"[DEBUG] Conf after adjustment: min={np.min(predictions.conf):.4f}, max={np.max(predictions.conf):.4f}, mean={np.mean(predictions.conf):.4f}")
    ...
    def process_long_sequence(self):
        ...
        # Handle single chunk case - no alignment needed but still save PLY
        if len(self.chunk_indices) == 1:
            ...
            print(f"[DEBUG] Confidence stats - min: {np.min(confs_first):.4f}, max: {np.max(confs_first):.4f}, mean: {np.mean(confs_first):.4f}")
            conf_threshold = max(0.0, np.mean(confs_first) * self.config["Model"]["Pointcloud_Save"]["conf_threshold_coef"])
            print(f"[DEBUG] Using conf_threshold: {conf_threshold:.4f}")
            save_confident_pointcloud_batch(
    ...
    def save_camera_poses(self):
        ...
        # When there's only one chunk, don't exclude overlap at the end
        first_chunk_end = first_chunk_range[1] - self.overlap_e if len(self.all_camera_poses) > 1 else first_chunk_range[1]
        ...
        with open(poses_path, "w") as f:
            for pose in all_poses:
                if pose is None:
                    print("Warning: Skipping None pose")
                    continue
        ...
        # Filter out None poses for proper PLY header
