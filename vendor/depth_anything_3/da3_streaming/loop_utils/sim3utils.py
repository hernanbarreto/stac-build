...
def save_confident_pointcloud_batch(
    ...
    # Check if confidence values are uniform (no meaningful confidence data)
    # In this case, include all points regardless of confidence
    conf_is_uniform = np.allclose(np.min(confs), np.max(confs), rtol=1e-5)
    print(f"[DEBUG save_confident_pointcloud_batch] conf_is_uniform={conf_is_uniform}, min={np.min(confs):.6f}, max={np.max(confs):.6f}")
    
    def get_mask(cfs, conf_threshold, conf_is_uniform):
        """Get mask for valid points based on confidence"""
        if conf_is_uniform:
            # No meaningful confidence - include all points
            return np.ones(len(cfs), dtype=bool)
        else:
            # Normal case: filter by confidence threshold and minimum value
            return (cfs >= conf_threshold) & (cfs > 1e-5)

    total_valid = 0
    for i in range(b):
        cfs = confs[i].reshape(-1)
        mask = get_mask(cfs, conf_threshold, conf_is_uniform)
        total_valid += np.count_nonzero(mask)
