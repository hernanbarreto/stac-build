...
# ── STEPS 9-10 — every closure is a CONSTRAINT, and the curve satisfies them ─
#
# USER 2026-09-18: *"podrian existir varias zonas separadas con candidatos de
# ajuste … si son de regiones diferentes no deben competir por el vector
# correccion … lo mas sano es ajustar la posicion de cada uno al vector que se
# determina y no necesariamente esperar que haya un unico vector que solucione
# todo, porque termina no solucionando a nadie"*.
#
# A closure is not a vector to apply, it is a MEASUREMENT OF A DIFFERENCE. The
# object was at ``d_a`` when one visit saw it and at ``d_b`` when the other did,
# and the two copies differ by what the walk accumulated in between:
#
#     E(d_a) - E(d_b) = t_i
#
# with ``E(0) = 0`` — the start is exact. Written this way the question "are
# these two closures in the same region?" does not need a threshold to answer:
#
#   · two closures over the SAME stretch measure the same number. If they
#     agree they confirm each other; if they do not, one of them is wrong and
#     the BEST DETERMINED wins (criterion 23 — never the average, which on pccr
#     produced an epoch worse than the one before it).
#   · two closures over DIFFERENT stretches measure different things and never
#     compete: each constrains its own piece of the curve.
#
# So the closures are the edges of a graph whose nodes are the chainages they
# touch, and the solution is a SPANNING FOREST built best-determined-first: a
# closure that would close a cycle is redundant, and what it disagrees with the
# already-determined curve by is reported, not averaged in.
#
# Only translation. No rotation is ever applied from fewer than three
# well-spread, non-collinear objects (criterion 24), so the curve is a vector
# field in R3 and interpolates linearly — no screw needed.


@dataclass
class Constraint:
    """One closure, as what it actually measures."""

    label: str
    instance_id: int
    d_a: float                     # chainage of the reference visit
    d_b: float                     # chainage of the displaced visit
    t: np.ndarray                  # world vector taking copy B onto copy A
    disagreement_m: float          # how well determined it is
    common_voxels: int

    def as_dict(self) -> dict:
        return {"label": self.label, "instance_id": self.instance_id,
                "d_a_m": round(self.d_a, 3), "d_b_m": round(self.d_b, 3),
                "t_m": [round(float(x), 5) for x in self.t],
                "magnitude_m": round(float(np.linalg.norm(self.t)), 5),
                "disagreement_m": round(self.disagreement_m, 5),
                "common_voxels": int(self.common_voxels)}
...
def same_stretch(a: Constraint, b: Constraint, repeatability_m: float) -> bool:
    """Do two closures measure the same piece of the walk?

    Not a distance in metres — a MEASUREMENT. Two closures cover the stretches
    [d_a, d_b] and [d_a', d_b']; what one integrates and the other does not is
    ``|d_a - d_a'| + |d_b - d_b'|`` metres of walk. At the rate they themselves
    measure, that much walk accumulates a difference. When that difference is
    under what the session can REPEAT, no measurement could tell the two
    stretches apart and they are the same one.

    pccr: `desk#203` spans 0.55-18.75 m and `desk#201` 0.71-19.32 m. The
    0.73 m of walk between them accumulates 27 mm at the 37 mm/m they measure,
    against a repeatability of 47.7 mm — the same stretch. A revisit at 5 m and
    another at 15 m are not, by the same arithmetic.
    """
    span = max(abs(a.d_b - a.d_a), abs(b.d_b - b.d_a), 1e-9)
    rate = 0.5 * (float(np.linalg.norm(a.t)) + float(np.linalg.norm(b.t))) / span
    return rate * (abs(a.d_a - b.d_a) + abs(a.d_b - b.d_b)) <= float(repeatability_m)


def solve_drift_curve(constraints: Sequence[Constraint], chainage_kf: np.ndarray,
                      repeatability_m: float,
                      log: Callable[[str], None] = print
                      ) -> Tuple[np.ndarray, dict]:
    """Per-keyframe correction from the closures. ``t_kf[k]`` is what keyframe
    ``k`` must be moved by: ``-E(d_k)``, the accumulated error removed.

    Closures over the same stretch are ONE measurement made several times: the
    best determined wins and the rest are declared, never averaged (criterion
    23 — the average produced a pccr epoch worse than the one before it).
    Closures over different stretches never compete; each fixes the rate over
    its own piece of the walk.
    """
    chain = np.asarray(chainage_kf, np.float64)
    if not constraints:
        return np.zeros((len(chain), 3)), {"groups": [], "redundant": [],
                                           "provenance": "tool_measured"}

    # best determined first, so the representative of every group is the best
    order = sorted(constraints, key=lambda c: (c.disagreement_m, -c.common_voxels))
    groups: List[List[Constraint]] = []
    for c in order:
        for g in groups:
            if same_stretch(g[0], c, repeatability_m):
                g.append(c)
                break
        else:
            groups.append([c])

    # one rate per group, from its best-determined member alone
    rates = []
    for g in groups:
        best = g[0]
        span = best.d_b - best.d_a
        if abs(span) < 1e-6:
            continue
        # E(d_a) - E(d_b) = t   with  E(d) = r*d   →   r = t / (d_a - d_b)
        rates.append({"lo": min(best.d_a, best.d_b), "hi": max(best.d_a, best.d_b),
                      "r": np.asarray(best.t, np.float64) / (best.d_a - best.d_b),
                      "best": best, "members": g})
    if not rates:
        return np.zeros((len(chain), 3)), {"groups": [], "redundant": [],
                                           "provenance": "tool_measured"}
    rates.sort(key=lambda x: x["lo"])

    # the rate profile over the walk: each group owns its stretch; a stretch no
    # closure covers inherits the nearest measured rate, which is the model's
    # own sentence — the error keeps accumulating at the rate last measured
    edges = sorted({0.0, float(chain[-1])} | {r["lo"] for r in rates}
                   | {r["hi"] for r in rates})
    prof = []
    for k in range(len(edges) - 1):
        a, b = edges[k], edges[k + 1]
        mid = 0.5 * (a + b)
        own = [r for r in rates if r["lo"] <= mid <= r["hi"]]
        if own:
            r = own[0]["r"]                       # best determined claims it
        else:
            near = min(rates, key=lambda r: min(abs(mid - r["lo"]),
                                                abs(mid - r["hi"])))
            r = near["r"]
        prof.append((a, b, r))

    def _E(d: float) -> np.ndarray:
        acc = np.zeros(3)
        for a, b, r in prof:
            if d <= a:
                break
    ...
    t_kf = -np.stack([_E(float(d)) for d in chain])

    red = []
    for g in groups:
        for c in g[1:]:
            want = np.asarray(c.t, np.float64)
            got = _E(c.d_a) - _E(c.d_b)
            red.append({**c.as_dict(), "same_stretch_as": g[0].instance_id,
                        "curve_says_m": [round(float(x), 5) for x in got],
                        "differs_by_m": round(float(np.linalg.norm(want - got)), 5)})
    rep = {
        "groups": [{"chainage_from_m": round(r["lo"], 3),
                    "chainage_to_m": round(r["hi"], 3),
                    "decided_by": f"{r['best'].label}#{r['best'].instance_id}",
                    "rate_mm_per_m": round(float(np.linalg.norm(r["r"])) * 1000, 2),
                    "members": [f"{c.label}#{c.instance_id}" for c in r["members"]]}
                   for r in rates],
        "redundant": red,
        "max_correction_m": round(float(np.linalg.norm(t_kf, axis=1).max()), 5),
        "max_step_between_keyframes_mm": round(float(
            np.linalg.norm(np.diff(t_kf, axis=0), axis=1).max() * 1000), 3)
        if len(t_kf) > 1 else 0.0,
        "provenance": "tool_measured"}
    log(f"[visit-drift] curve: {len(rates)} stretch(es) of the walk, decided by "
        + ", ".join(g["decided_by"] for g in rep["groups"])
        + f"; {len(red)} closure(s) redundant; max correction "
        f"{rep['max_correction_m'] * 100:.1f} cm, step between keyframes "
        f"{rep['max_step_between_keyframes_mm']:.2f} mm")
    return t_kf, rep


# ── STEP 9-10 (JOINT) — the objects and the floor in ONE system ──────────
#
# USER 2026-09-19, after the object correction reopened a floor the chunk
# levelling had just closed: *"se arreglo el desk, pero se duplico y rompio el
# piso en esa zona"*.
#
# Measured: the chunk edge 0<->6 — the two ends of the walk, which are exactly
# the keyframes of the desk's two visits — went from 5.8 mm to 73.3 mm the
# moment the desk's 68.7 cm was applied. The other five edges barely moved.
#
# It is the SAME collision, for the third time in one session:
#
#   · the desk against the glass doors        — they share keyframes
#   · the per-keyframe floor against the desk — they share keyframes
#   · the chunk floor against the desk        — they share keyframes
#
# Applying one correction after another always breaks the first, whatever the
# order and whichever is better. The correction is per keyframe, and two
# constraints on the same keyframe cannot be satisfied one after the other.
#
# So both families enter ONE system over the same unknowns:
#
#     object:  E(d_a) - E(d_b) = t              full vector, between two moments
#     floor:  (E(c_i) - E(c_j)) . up = d_ij     vertical only, between two chunks
#
# They barely compete: the floor constrains one direction and the objects are
# mostly horizontal. Solved together the desk closes AND the floor edges stay
# where the chunk levelling put them.
#
# Criterion 23 still holds inside the object family: closures over the same
# stretch are one measurement and the BEST DETERMINED decides — never an
# average.


def solve_joint_curve(constraints: Sequence[Constraint],
                      floor_dz: Optional[np.ndarray],
                      floor_constrained: Optional[np.ndarray],
                      up: np.ndarray, chainage_kf: np.ndarray,
                      repeatability_m: float,
                      texture_edges: Optional[Sequence[dict]] = None,
                      chunk_kf: Optional[Sequence[int]] = None,
                      log: Callable[[str], None] = print
                      ) -> Tuple[np.ndarray, dict]:
    """Per-keyframe correction from the objects AND the floor, in ONE system.

    The unknown is ``E[k]``, the accumulated error of every keyframe:

        object:  E[k_a] - E[k_b] = t           full vector, between two moments
        floor:   E[k] . up       = dz[k]       vertical only, per keyframe
        rate:    second difference of E along the walk -> 0   (the model)
        gauge:   mean E = 0                    only differences are observed

    The floor enters AT THE RESOLUTION WHERE THE DAMAGE HAPPENS. Six chunk
    edges were not enough: they left keyframes 0-12 and 200-215 — exactly where
    the desk's correction acts — with no constraint at all, and the floor's own
    dispersion went 66.4 -> 123.2 mm at long separations while every chunk edge
    stayed exact (pccr 2026-09-19).

    The two families barely compete: the floor fixes one direction and the
    objects are mostly horizontal. What determines the horizontal shape is the
    rate term, which is the drift-rate model written as an equation — with one
    object closure and smoothness, ``E`` comes out a straight line in chainage,
    which is ``E(d) = ε·d``.
    ...
    def _row():
        return np.zeros(3 * n_kf)

    def _kf_at(d: float) -> int:
        """The keyframe whose chainage a constraint refers to."""
        return int(np.argmin(np.abs(chain - float(d))))

    for c in kept:
        ka, kb = _kf_at(c.d_a), _kf_at(c.d_b)
        if ka == kb:
            continue
        weight = np.sqrt(max(c.common_voxels, 1)) / max(c.disagreement_m,
                                                        repeatability_m)
        for comp in range(3):
            r = _row()
            r[3 * ka + comp] = 1.0
            r[3 * kb + comp] = -1.0
            rows.append(r)
            rhs.append(float(c.t[comp]))
            w.append(weight)
    n_obj = len(rows)

    n_floor = 0
    if floor_dz is not None and floor_constrained is not None:
        fw = 1.0 / max(repeatability_m, 1e-6)
        for k in np.flatnonzero(np.asarray(floor_constrained)):
            r = _row()
            r[3 * int(k):3 * int(k) + 3] = up
            rows.append(r)
            rhs.append(float(floor_dz[int(k)]))
            w.append(fw)
            n_floor += 1

    # THE FLOOR'S TEXTURE — the only horizontal the floor can give. A drift
    # ALONG the floor moves no point off it, so its geometry is blind to it;
    # what is painted on it is not (USER 2026-09-19: "hay desacuerdo pero entre
    # las lineas del piso"). Two equations per pair, in the plane orthogonal to
    # `up`, weighted by shared cells AND by how well the correlation locked —
    # a floor is a weak target and a pair that did not lock must not outvote
    # one that did.
    n_tex = 0
    if texture_edges and chunk_kf is not None:
        e1 = np.cross(up, [1.0, 0.0, 0.0])
        if np.linalg.norm(e1) < 1e-6:
            e1 = np.cross(up, [0.0, 1.0, 0.0])
        e1 = e1 / np.linalg.norm(e1)
        e2 = np.cross(up, e1)
        for ed in texture_edges:
            i, j = ed["pair"]
            if i >= len(chunk_kf) or j >= len(chunk_kf):
                continue
            ki, kj = int(chunk_kf[i]), int(chunk_kf[j])
            if ki == kj:
                continue
            sh = np.asarray(ed["shift_m"], np.float64)
            wt = (np.sqrt(max(ed["shared_cells"], 1)) * max(ed["peak"], 0.0)
                  / max(repeatability_m, 1e-6))
            for axis in (e1, e2):
                r = _row()
                r[3 * ki:3 * ki + 3] = axis
                r[3 * kj:3 * kj + 3] = -axis
                rows.append(r)
                rhs.append(float(sh @ axis))
                w.append(wt)
                n_tex += 1

    # THE MODEL'S OWN PREMISE, as equations: the error accumulates with the
    # distance walked, so its RATE varies slowly. It is also what determines
    # the horizontal, which only one closure touches. A tie-breaker where the
    # data speaks and the model where it does not — the residuals below say
    # which happened.
    lam = (min(w) if w else 1.0) * 1e-3
    for k in range(1, n_kf - 1):
        h0 = max(chain[k] - chain[k - 1], 1e-9)
        h1 = max(chain[k + 1] - chain[k], 1e-9)
        for comp in range(3):
            r = _row()
            r[3 * (k + 1) + comp] = 1.0 / h1
            r[3 * k + comp] = -(1.0 / h1 + 1.0 / h0)
            r[3 * (k - 1) + comp] = 1.0 / h0
            rows.append(r)
            rhs.append(0.0)
            w.append(lam)
    ...
    for comp in range(3):
        ...
        rows.append(r)
        rhs.append(0.0)
        w.append(np.sqrt(len(rows)))

    A = np.asarray(rows)
    y = np.asarray(rhs)
    wv = np.asarray(w)
    x = np.linalg.lstsq(A * wv[:, None], y * wv, rcond=None)[0]
    E = x.reshape(n_kf, 3)
    t_kf = -E

    obj_res = []
    for c in kept:
        got = E[_kf_at(c.d_a)] - E[_kf_at(c.d_b)]
        obj_res.append({**c.as_dict(),
                        "solved_m": [round(float(v), 5) for v in got],
                        "residual_m": round(float(np.linalg.norm(c.t - got)), 5)})
    fl_res = None
    if n_floor:
        kk = np.flatnonzero(np.asarray(floor_constrained))
        err = (E[kk] @ up) - np.asarray(floor_dz)[kk]
        fl_res = {"keyframes": int(n_floor),
                  "residual_mm": {"median": round(float(np.median(np.abs(err))) * 1000, 2),
                                  "p90": round(float(np.percentile(np.abs(err), 90)) * 1000, 2),
                                  "max": round(float(np.abs(err).max()) * 1000, 2)}}

    rep = {"mode": "joint_per_keyframe", "smoothness_weight": float(lam),
           "object_equations": n_obj, "floor_equations": n_floor,
           "texture_equations": n_tex,
           "texture_edges": list(texture_edges or []),
           "object_constraints": obj_res, "floor": fl_res,
           "redundant": [{**c.as_dict(), "same_stretch_as": g[0].instance_id}
                         for g in groups for c in g[1:]],
           "max_correction_m": round(float(np.linalg.norm(t_kf, axis=1).max()), 5),
           "max_step_between_keyframes_mm": round(float(
               np.linalg.norm(np.diff(t_kf, axis=0), axis=1).max() * 1000), 3)
           if n_kf > 1 else 0.0,
           "provenance": "tool_measured"}
    log(f"[visit-drift] joint: {len(kept)} object closure(s) + {n_floor} "
        f"floor keyframe(s) + {n_tex} texture equation(s) over {n_kf} "
        f"unknowns; max correction "
        f"{rep['max_correction_m'] * 100:.1f} cm, step between keyframes "
        f"{rep['max_step_between_keyframes_mm']:.2f} mm")
    for o in obj_res:
        log(f"[visit-drift]   {o['label']}#{o['instance_id']}: asked "
            f"{o['magnitude_m'] * 100:.1f} cm, residual "
            f"{o['residual_m'] * 100:.1f} cm")
    if fl_res:
        log(f"[visit-drift]   floor: residual median "
            f"{fl_res['residual_mm']['median']:.1f} mm, p90 "
            f"{fl_res['residual_mm']['p90']:.1f} mm, max "
            f"{fl_res['residual_mm']['max']:.1f} mm")
    return t_kf, rep
