...
Input: a JSON spec written by server/fuse_scans.py:
    ...
    {"reference": {"pairs": {label: "path.npy", ...}},
...
def size_measure(P, planes):
    """Object size from its own geometry: median of plane-to-plane center
    spreads when ≥2 planes, else robust radius (median distance to
    centroid). Independent of mask extent extremes."""
    if len(planes) >= 2:
        d = []
        for i in range(len(planes)):
            for j in range(i + 1, len(planes)):
                d.append(float(np.linalg.norm(planes[i][1] - planes[j][1])))
        d = [x for x in d if x > 0.05]
        if d:
...
def register_scan(ref_pairs, scan_pairs, params, log):
    ...
    res = cc.ICP(data, model, adjustScale=True, finalOverlapRatio=overlap,
    ...
    # per-pair residuals are computed by the orchestrator (scipy KD-tree)
    return {"T": T.reshape(-1).tolist(), "T_coarse": T0.reshape(-1).tolist(),
            "scale": sc, "rms_cm": float(res.finalRMS * 100),
