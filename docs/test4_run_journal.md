...
| A4 | pending | — | `intra_chunk: false` (drift+elastic back to run-6 true), "run 3 behaviour" | | question: serpenteado gone? Last per-frame warp stage. If it persists → cause is omega's raw feed-forward output (Phase C/E territory) |
