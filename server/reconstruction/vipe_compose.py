...
calibrated depth with its ViPE c2w pose into world space, emitting chunk PLYs +
chunk_*_origins.npz (frame_global, pixel_row, pixel_col, confidence) 1:1 with the
points — the SAME schema map_worker._generate_origins / CloudComPy expect, so the
downstream merge + cleaned_cloud + TSDF are unchanged.

Replaces DA3's world_points chunks when pose_source == "vipe". frame_global is the
ORIGINAL frame number (all-frames), not a keyframe index.

Runs in the da3 env (numpy + open3d; calibrated depth is plain npz). No EXR/ViPE.
"""
import json
...
def compose_chunks_from_vipe(
    output_dir: Path,
    frames_dir: Path,
    cal_dir: Path,                       # vipe_run/depth_calibrated (npz: depth,intrinsics,conf)
    ...
    """Compose chunk_*.ply + chunk_*_origins.npz from calibrated ViPE depth. Returns
    the number of chunks written. Points/origins are written 1:1 from the same mask."""
    output_dir = Path(output_dir); frames_dir = Path(frames_dir); cal_dir = Path(cal_dir)
    # only frames with both a calibrated depth and a pose
    frames = sorted(f for f in pose_map
                    if (cal_dir / f"{int(f):06d}.npz").exists())
    if not frames:
        raise RuntimeError("[vipe-compose] no frames with calibrated depth + pose")
