...
def _cloudcompy_work(pipe: WorkerPipe, session_dir: str, config: dict):
    ...
    # Include LiDAR complement cloud if present (generated by hybrid mode)
    lidar_complement = output_dir / "lidar_complement.ply"
    if lidar_complement.exists():
        chunks.append(lidar_complement)
        pipe.send_log(f"Including LiDAR complement cloud in merge ({lidar_complement.stat().st_size / 1048576:.0f} MB)")
