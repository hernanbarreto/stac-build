...
after ViPE + the depth/scale step have produced vipe_run/ + vipe_depth/.
...
def main():
    ...
    # frame numbers that have a metric ViPE depth map
    depth_frames = sorted(int(p.stem) for p in depth_dir.glob("[0-9]*.npz"))
    pose_map = vipe_poses.build_pose_map(vipe_out, frames=depth_frames, scale=scale)
    print(f"[vipe-compose] pose_map: {len(pose_map)} frames", flush=True)

    # per-frame intrinsics for the pose files (from the ViPE depth npz K)
    intr = {}
    for fr in pose_map:
    ...
    n = vipe_compose.compose_chunks_from_vipe(
        output_dir=out, frames_dir=args.frames_dir, depth_dir=depth_dir,
