...
class MemoryEfficientTextFeatureExtractor:
    """Reconstruido 29-05-2026. Precomputa T5+CLIP y libera VRAM antes de la inferencia 3D (anti-OOM). Drop-in con TextFeatureExtractor."""
    ...
    @torch.no_grad()
    def precompute_and_free(self, captions):
        self.t5 = load_t5(self.device)
        self.clip = load_clip(self.device)
        if self._dtype is not None:
            self.t5 = self.t5.to(self._dtype)
            self.clip = self.clip.to(self._dtype)
        unique_caps = list(dict.fromkeys(captions))
        for cap in unique_caps:
            t5_out = self.t5([cap]).detach().to("cpu")
            clip_out = self.clip([cap]).detach().to("cpu")
            self._cache[cap] = (t5_out, clip_out)
        del self.t5
        del self.clip
        self.t5 = None
        self.clip = None
    ...
    def __call__(self, captions):
        ...
        for cap in captions:
            if cap not in self._cache:
                raise KeyError(f"Caption no precomputado: {cap!r}")
            t5_c, clip_c = self._cache[cap]
            t5_list.append(t5_c.to(self.device))
            clip_list.append(clip_c.to(self.device))
        return torch.cat(t5_list, dim=0), torch.cat(clip_list, dim=0)
