...
def _disk_session(out, rng_seed=41, bias=None):
    """Two healthy room chunks on disk (chunk_000/chunk_001 + origins + poses).
    chunk_001 optionally carries a rigid ``bias`` so finereg has work to do."""
    import numpy as np
    from plyfile import PlyData, PlyElement
    ...
    a = make_room_chunk(n_per_surf=15_000, seed=42)
    b = make_room_chunk(n_per_surf=15_000, seed=43)
    if bias is not None:
        b = b @ bias[:3, :3].T + bias[:3, 3]
    write_chunk(0, a, rng.integers(0, 10, len(a)))
    write_chunk(1, b, rng.integers(10, 20, len(b)))
    (out / "camera_frames.txt").write_text(" ".join(str(f) for f in range(20)))
    eye = " ".join(f"{x:.9g}" for x in np.eye(4).reshape(-1))
    (out / "camera_poses.txt").write_text("\n".join([eye] * 20) + "\n")


class TestHealthIndexMapping:
    def test_sick_source_indices_map_through_meta(self, tmp_path):
        """pccr 2026-07-11 regression: chunk_health.json speaks ORIGINAL vendor
        indices; after the sick chunk 0 is excluded the PLYs are renumbered
        (chunk_000 = source 1). Down-weighting must follow source_chunk from
        the metas — here the sick source wrote no PLY, so NOBODY is weak."""
        import json
        from reconstruction.surface_fit.fine_register import run as run_finereg
        out = tmp_path
        _disk_session(out, bias=rigid(t=(0.006, -0.004, 0.005)))
        for cid, src in [(0, 1), (1, 2)]:                 # dense → source mapping
            (out / f"chunk_{cid:03d}_meta.json").write_text(
                json.dumps({"chunk_id": cid, "source_chunk": src}))
        (out / "maplong_run").mkdir()
        (out / "maplong_run" / "chunk_health.json").write_text(
            json.dumps({"sick": {"0": ["broken start"]}, "suspect": {}}))
        run_finereg(out, accept_sep_m=0.024, max_correction_m=0.2,
                    pieces_per_chunk=1)
        rep = json.loads((out / "fine_register_report.json").read_text())
        assert rep["unit_conf"] == {}                     # sick chunk wrote no PLY
        assert not rep["rolled_back"]

    def test_suspect_source_lands_on_renumbered_chunk(self, tmp_path):
        import json
        from reconstruction.surface_fit.fine_register import run as run_finereg
        out = tmp_path
        _disk_session(out, bias=rigid(t=(0.006, -0.004, 0.005)))
        for cid, src in [(0, 1), (1, 2)]:
            (out / f"chunk_{cid:03d}_meta.json").write_text(
                json.dumps({"chunk_id": cid, "source_chunk": src}))
        (out / "maplong_run").mkdir()
        (out / "maplong_run" / "chunk_health.json").write_text(
            json.dumps({"sick": {}, "suspect": {"2": ["shaky"]}}))
        run_finereg(out, accept_sep_m=0.024, max_correction_m=0.2,
                    pieces_per_chunk=1)
        rep = json.loads((out / "fine_register_report.json").read_text())
        # source 2 = dense chunk 1: only its units are down-weighted
        assert rep["unit_conf"] and all(k.startswith("(1,")
                                        for k in rep["unit_conf"])

    def test_no_meta_keeps_identity_mapping(self, tmp_path):
        import json
        from reconstruction.surface_fit.fine_register import run as run_finereg
        out = tmp_path
        _disk_session(out, bias=rigid(t=(0.006, -0.004, 0.005)))
        (out / "maplong_run").mkdir()
        (out / "maplong_run" / "chunk_health.json").write_text(
            json.dumps({"sick": {}, "suspect": {"0": ["shaky"]}}))
        run_finereg(out, accept_sep_m=0.024, max_correction_m=0.2,
                    pieces_per_chunk=1)
        rep = json.loads((out / "fine_register_report.json").read_text())
        assert rep["unit_conf"] and all(k.startswith("(0,")
                                        for k in rep["unit_conf"])


class TestDegradationRollback:
    def test_degrading_solve_rolled_back_to_identity(self, tmp_path, monkeypatch):
        """pccr 2026-07-11 regression: the solve degraded the worst separation
        (91.4mm → 116.7mm) and was applied anyway, serpentining straight
        cabinet rows. A solve that comes out worse must leave the cloud and
        the poses EXACTLY as they were."""
        import json
        import numpy as np
        from plyfile import PlyData
        from reconstruction.surface_fit import fine_register as fr
        out = tmp_path
        _disk_session(out)                                # already aligned: any move hurts

        harmful = rigid(rz=np.deg2rad(0.5), t=(0.05, -0.04, 0.03))
        monkeypatch.setattr(fr, "solve_joint",
                            lambda planes, anchor, **kw: {
                                u: (harmful.copy() if u[0] == 1 else np.eye(4))
                                for u in planes})

        before = np.array(PlyData.read(str(out / "chunk_001.ply"))["vertex"].data)
        poses_before = (out / "camera_poses.txt").read_text()
        n = fr.run(out, accept_sep_m=0.024, max_correction_m=0.2,
                   pieces_per_chunk=1)
        assert n == 0                                     # nothing applied
        rep = json.loads((out / "fine_register_report.json").read_text())
        assert rep["rolled_back"] and not rep["accepted"]
        for M in rep["corrections"].values():             # all identity
            assert np.allclose(np.asarray(M), np.eye(4))
        assert rep["sep_after_m"] == rep["sep_before_m"]  # cloud state honest
        assert max(rep["sep_rejected_m"].values()) > max(rep["sep_before_m"].values())
        after = np.array(PlyData.read(str(out / "chunk_001.ply"))["vertex"].data)
        assert np.array_equal(before, after)              # points untouched
        assert (out / "camera_poses.txt").read_text() == poses_before
        assert not (out / "camera_poses.txt.prefinereg").exists()
