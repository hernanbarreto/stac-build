...
**CURRENT BEST (2026-07-12): G1** = run6 + elastic taming (B) + ownership backfill
+ conf 10% (D), pose_refine/hybrid/finereg OFF — run6 solidity with +22% density
(29.4M vs 24M). User: "buen resultado, densidad y poses".
...
| F1 | 2026-07-11 23:02 | 508f327 | base = run6 + B + D (hybrid_da3 OFF, fine_register OFF) + **E-full pose_refine** | Base clean: hybrid 0 lines, finereg 0 lines, deterministic stages = run6 (scales 40.4445/0.9709, anchor 0.78→0.38%, seams 6.6…8.4). B active (33 fits capped), D active (3.73M px backfilled). pose_refine: 210 frames, 1965 edges, 774,584 matches; layering measured 18.9 mm med / 52.5 p90 → **9.1 mm fresh (−52%)**; corrections median 160 mm max 444 mm; 210/210 poses applied. Cloud 45.7M merged | ⚖ user: **run 6 REMAINS THE BEST overall.** F1 is the best of the new attempts: better density, MUCH better drift correction than E2, somewhat less serpenteado, more coverage — but still some drift, e.g. the chimney around image ~704 (chunk-3 landmark) drifts in F1 while in run 6 it was solid ("clavada") |
| F2 | 2026-07-11 23:56 | 1867dd9 | pose_refine far-field: max_depth 8→30 m with (8/z)² weights, pair_window 15 | 2568 edges / 996k matches (far field now measured: initial 25.0 mm med / 75.6 p90). Fresh 25.0→11.5 mm (−54%), corrections 215 mm med / 476 max, 210/210 applied | ❌ user: NOT better than run 6; only clear win is COVERAGE/density. Chimney (~704) STILL displaced — far-field weights didn't fix it. **pose_refine does not pay visually vs run6 on this scene** |
| G1 | 2026-07-12 00:33 | 7efcb63 | `pose_refine.enabled: false` — run6 geometry (poses untouched) + Phase D density (backfill + conf 10) + B | 0 pose-refine lines ✓. Deterministic = run6 (scales 40.4437/0.9709, anchor 0.78→0.38%, seams 6.6…8.4). Backfill 3.73M px. Cloud 45.7M merged → **29.4M cleaned** (run6 was ~24M) | ✅✅ user: "buen resultado, densidad y poses" — **NEW BEST: run6 solidity + F2 coverage. This is the production baseline now** |
