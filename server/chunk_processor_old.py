...
class ChunkProcessor:
    ...
    MODEL_NAME = cfg.get("models", {}).get("depth", {}).get("name", "depth-anything/DA3-LARGE-1.1")
    
    # Resolve weights dir (relative to server or absolute)
    _weights_path_str = cfg.get("paths", {}).get("da3_weights_dir", "../Depth-Anything-3/da3_streaming/weights")
    WEIGHTS_DIR = Path(__file__).parent / _weights_path_str
    
    CONFIG_FILE = WEIGHTS_DIR / cfg.get("models", {}).get("depth", {}).get("config_file", "config.json")
    WEIGHTS_FILE = WEIGHTS_DIR / cfg.get("models", {}).get("depth", {}).get("weights_file", "da3-large-1.1.safetensors")
    
    def __init__(self, device: str = None):
        # Allow config to override device
        cfg_device = cfg.get("models", {}).get("depth", {}).get("device", "auto")
        
        if cfg_device != "auto":
            self.device = cfg_device
            if self.device == "cpu":
                 print(f"[ChunkProcessor] ⚠️ Forced CPU mode from config.")
        else:
            # Auto fallback (original logic)
            if "GIANT" in self.MODEL_NAME.upper():
                print(f"[ChunkProcessor] ⚠️ GIANT model detected (Auto). Forcing CPU mode to avoid VRAM OOM on RTX 3060.")
                print(f"[ChunkProcessor] Expect SLOW processing (5-10 mins/chunk).")
                self.device = "cpu"
            else:
                 # If device passed in init is valid, use it, else default to cuda
                 target = device if device else ("cuda" if torch.cuda.is_available() else "cpu")
                 self.device = target
        
        # Inference Precision (Formerly Hardcoded float16)
        prec_str = cfg["models"]["depth"]["inference_precision"]
        self.dtype = getattr(torch, prec_str)
        
        # Torch Compile Mode (Formerly Hardcoded reduce-overhead)
        self.compile_mode = cfg["models"]["depth"]["torch_compile_mode"]
        
        # View Strategy (Formerly Hardcoded saddle_balanced)
        self.view_strategy = cfg["models"]["depth"]["ref_view_strategy"]
            ...
            with torch.no_grad():
                ...
                with torch.amp.autocast('cuda', dtype=self.dtype):
                    predictions = self.model.inference(
                        image_paths,
                        ref_view_strategy=self.view_strategy,
                    )
    ...
    def generate_point_cloud(self, result: ChunkResult, 
                            ...
                            conf_threshold_coef: float = None) -> np.ndarray:
        """
        Generate point cloud using VECTORIZED NumPy operations.
        KEYFRAME OPTIMIZATION: Processes only 1 out of every N frames (configured).
        """
        t0 = time.time()
        
        # Config Defaults (Expects valid config.yaml)
        pc_cfg = cfg.get("point_cloud", {}).get("generation", {})
        
        # Override if passed, else load from config
        if sample_ratio is None: 
            sample_ratio = pc_cfg.get("sample_ratio") 
            if sample_ratio is None: sample_ratio = 0.1 # Absolute fallback if config broken
            
        if conf_threshold_coef is None: 
            conf_threshold_coef = pc_cfg.get("conf_threshold_coef")
            if conf_threshold_coef is None: conf_threshold_coef = 0.75

        kf_interval = pc_cfg.get("keyframe_interval") or 5
        min_depth = pc_cfg.get("min_depth") or 0.1
        max_depth = pc_cfg.get("max_depth") or 100.0
        
        # --- NUEVA LÓGICA DE KEYFRAMES ---
        # Seleccionamos los índices según config
        indices_keyframes = np.arange(0, result.frame_count, kf_interval)
        ...
        # Filter (Using Configured Depths)
        mask = (confs_all >= conf_threshold) & (depths_all > min_depth) & (depths_all < max_depth)
